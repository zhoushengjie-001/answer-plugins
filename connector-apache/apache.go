...
func (g *Connector) ConnectorReceiver(ctx *plugin.GinContext, receiverURL string) (userInfo plugin.ExternalLoginUserInfo, err error) {
	...
	resp, err := http.Get("https://oauth.apache.org/token?code=" + code)
	if err != nil {
	...
	if err != nil {
		log.Errorf("read user info failed: %v", err)
		return userInfo, err
	}

	// parse user info
	apacheResp := OAuthResponse{}
	err = json.Unmarshal(body, &apacheResp)
	if err != nil {
		log.Errorf("parse user info failed: %v", err)
		return userInfo, err
	}

	metaInfo, _ := json.Marshal(apacheResp)
	userInfo = plugin.ExternalLoginUserInfo{
		ExternalID:  apacheResp.Uid,
		DisplayName: apacheResp.Fullname,
		Username:    apacheResp.Uid,
		Email:       apacheResp.Email,
